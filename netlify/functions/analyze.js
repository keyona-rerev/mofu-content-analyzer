...
- For each job, score 0-100 using these subweights: presence+count (0-40, scaled by how close the piece count is to a 3-piece minimum), audience fit (0-30, how well matched to the inferred buyer), job completion (0-30, does the content actually accomplish what the job needs, quote-worthy specifics over vague gestures).
...
Return ONLY a JSON object, no markdown fences, no preamble, exactly this shape:
{
  "composite_score": 41,
  "jobs": {
    "symptom": {"score": 65, "diagnosis": "1-3 sentences on what's working and what's missing, plain prose", "pieces": [{"url": "...", "title": "...", "fit": "strong|loose"}]},
  ...
  "unmatched": [{"url": "...", "title": "...", "reason": "1 sentence on why this doesn't count toward any job"}]
}
Every page you were given must appear exactly once, either inside one job's "pieces" array or inside "unmatched". Keep diagnosis strings under 500 characters each and reason strings under 200 characters each. No citation markers, brackets, or markdown inside any string.`;
}
...
exports.handler = async (event) => {
  ...
  let pages;
  try {
    pages = await crawl(seedUrl);
  } catch (e) {
    return json(502, { error: `Could not crawl that site: ${String(e.message || e).slice(0, 200)}` });
  }
  if (!pages.length) return json(502, { error: 'Crawled the site but found no readable content pages.' });
  ...
  const clean = { pages_crawled: pages.length, source_url: seedUrl, jobs: {}, unmatched: [] };
